...
def fit_model(model, train_data, validation_data, patience, epochs, batch_size, validation_split):
    ...
    # save the best model
    model_checkpoint_callback = ModelCheckpoint(filepath='/kaggle/working/best_model.hdf5',
...
def make_predictions(trained_model, X_test):
    ...
    X_test_scaled = X_test.reshape(len(X_test), 28, 28)
    X_test_tensor = X_test.reshape(len(X_test_scaled), 28, 28, 1)
...
df_submission_example = pd.read_csv('/kaggle/input/digit-recognizer/sample_submission.csv')
